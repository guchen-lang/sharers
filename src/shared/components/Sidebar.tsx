...
export default function Sidebar() {
  ...
  return (
    <aside className="w-72 p-4 border-r bg-white dark:bg-slate-900">
      ...
      {/* Static quick link to JSON Compare so the entry is always visible in the sidebar */}
      <div className="mb-3">
        <Link
          to="/json-compare"
          className={`flex items-center px-2 py-2 rounded hover:bg-slate-100 dark:hover:bg-slate-800 ${{
            true: 'font-medium'
          }[String(location.pathname === '/json-compare')]}`}
        >
          <ToolIcon Icon={Code} />
          <div>
            <div className="text-sm">JSON Compare</div>
            <div className="text-xs text-slate-500 dark:text-slate-400">Compare two JSON docs side-by-side</div>
          </div>
        </Link>
      </div>
